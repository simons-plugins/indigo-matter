...
| `settings_report.py` | What a device exposes that the plugin does **not** offer (issue #191) — Indigo-free and matter-server-free. Evidence is the device's own **AttributeList** cross-referenced against `matter_handlers/writable_attributes.py`, because writability is spec metadata that never appears on the wire. Suppresses three things or the INFO becomes wallpaper: settings already offered on that endpoint's Indigo type, attributes the plugin already drives as *controls* (`DRIVEN_ATTRIBUTES`, hand-maintained and held to a parity test that greps every handler for `MatterWrite(`), and Matter plumbing (`INFRASTRUCTURE_CLUSTERS`). `SurveyLog` is the once-per-device bookkeeping: one JSON string in `pluginPrefs` keyed by **node** — not a device prop, because a node is not an Indigo device (an FP300 is one node and four devices) and stamping props costs a comm restart per device. Keyed on firmware **and** the settable set, so a first snapshot taken mid-interview self-corrects |
| `matter_handlers/writable_attributes.py` | **GENERATED, checked in** — `{cluster: {attribute: name}}` for all 909 model attributes plus the 110 the spec calls writable. Regenerate with `tools/enumerate_settings.py --emit-table` when `bridge-node/package.json`'s `@matter/*` pin moves. A build artefact rather than a runtime lookup because only the bridge node may import `@matter/model` (ADR-0006) |
