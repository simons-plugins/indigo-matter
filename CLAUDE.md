...
| `matter_handlers/` | One `ClusterHandler` per cluster + registry (OnOff in v1). `settings.py` is the **declarative registry of writable settings** — each names its cluster/attribute, the attribute its bounds come from, and how to parse them. Adding a setting is a declaration here plus its ConfigUI field(s) in `Devices.xml`: Indigo builds ConfigUI from static XML per device *type* and offers no runtime field injection (unlike states, cf. `getDeviceStateList`), so the XML cannot be generated from the declarations and the two are kept in step by hand. The presence of a setting's **limits** attribute is also its capability check, which is what makes a pre-1.4 occupancy sensor show no hold-time field rather than one that would fail. Issue #85's "Set Sensitivity Level" action and the dialog are two entry points onto this one mechanism — the action survives because it does a job the dialog cannot (changing a setting from an automation), but since #186 it no longer echoes optimistically |
