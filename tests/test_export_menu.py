...
class TestGetMigrateSources:
    ...
    round trip needed at all — the allow-list IS the export list."""
    ...
    def test_it_says_so_when_it_fails_outright(self, plug, plugin_mod, monkeypatch):
        ...
        already has, so a broken store degrades to one error row rather than
...
class TestGetMigrateDevices:
    """§3.2 — the target picker: uses `_migrate_device_row`'s per-device
    rule, keyed off the SOURCE ENTRY's role, plus one migrate-only rule:
    the source device itself is absent."""
