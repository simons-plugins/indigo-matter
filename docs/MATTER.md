...
Setup steps are in [INSTALL.md](./INSTALL.md); this section is the shape of it.
...
versioned: a bridge node from a different plugin version is refused rather than
guessed at.
...
The practical version of all that: **back up
...
the two destructive repair actions.
...
| Dimmer, full colour | **Full-colour light** *(default)*, Colour-temperature light, Dimmable light, Window covering | Extended Color Light |
...
| Not exportable | Why |
| --- | --- |
| Anything this plugin created | Loop guard: a Matter device is not re-exported over Matter. These are filtered out entirely. |
...
| Devices with no resolvable role — including this plugin's own energy-meter type | Nothing Matter can represent faithfully. |
...
- **Units are taken on trust.** Indigo declares no units anywhere, so a reading
  is assumed to already be in the unit Matter wants (°C, %RH, lux, kPa, m³/h).
  ...
  exception the plugin can be sure of, and it is converted.)
