...
- **`indigo.*` from the loop thread is the existing house discipline**, not a new risk: `device_sync.apply_states` already writes Indigo state from the loop, and `on_command` is deliberately the single seam that would have to move to `run_in_executor` if that ever changes.
