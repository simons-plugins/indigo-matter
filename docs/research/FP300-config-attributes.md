...
- **Sensitivity is already wired.** `matter_handlers/boolean_state_config.py`
  ...
  Level" custom device action writes it (issue #85). **Direction is settled by the
  spec, not by observation:** Matter cluster §1.8.6.2 says levels *shall* be ordered
  by sensitivity with **0 = lowest (least sensitive)** and the highest value most
  sensitive — so 0/1/2 here is least → most. §1.8.6.1 adds that writing an
  unsupported value *shall* return `CONSTRAINT_ERROR`. Spec text is quotable from
  `@matter/model/.../boolean-state-configuration.resource.js` on the server, which
  carries `details` + `xref` per attribute.
