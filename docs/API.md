...
> resolve the reflector, build `{base}/message/com.simon.indigo-matter/{handler}[/{pathArg}]`,
> add `Authorization: Bearer {credential}`, pass parameters as URL query items.
...
- **Base path:** every endpoint is rooted at `…/message/com.simon.indigo-matter/{handler}`.
  - Local: `http://{indigo-host}:8176/message/com.simon.indigo-matter/{handler}`
  - Remote (Reflector): `https://{reflector}.indigodomo.net/message/com.simon.indigo-matter/{handler}`
- Trailing path components after `{handler}` are positional arguments (the plugin reads them from `action.props["file_path"]`). E.g. `…/commission/{jobId}` puts `jobId` as the path arg.
...
### 3.4 `POST …/message/com.simon.indigo-matter/decommission/{nodeId}`

Decommission a device. `nodeId` is the trailing path component. Removes it from the Indigo fabric (sends `RemoveFabric` via matter-server) and deletes the associated Indigo devices.

> v1.0 specified this as `DELETE /matter/devices/{nodeId}`. IWS exposes only GET/POST to
> plugin handlers, so it is a **POST** in v1.1. Semantics are identical and idempotent.

**Request:** no parameters beyond the `nodeId` path component.
...
Diagnostic snapshot for a single device. `nodeId` is the trailing path component. Used by a future Domio device-detail view.
...
This contract is v1.1 (transport change from v1.0; semantics unchanged). Breaking changes require a version bump and a transition period where both versions are supported. Additive changes (new optional params, new handlers) do not require a version bump.

Domio includes `X-Matter-API-Version: 1.1` in every request. The plugin SHOULD warn (HTTP 200, with `warning` field) if Domio's version is unrecognised.
