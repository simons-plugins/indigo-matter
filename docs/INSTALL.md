...
**Reset Matter Bridge Pairings… — recoverable by pairing again.**
It *removes **every** ecosystem pairing from the Matter bridge and starts
...
> **Never use Indigo's "Edit Device Type" menu on a Matter device.** Indigo
> changes the type the *instant* you pick it — before you save, and even if you
> cancel — and a device left that way is marked *not configured*. Indigo then
> hides unconfigured devices from the plugin entirely, so it stops receiving
> updates and commands while still occupying its name. The plugin cannot
> prevent this, because it never gets told: the change happens before any save
> it could refuse, and the hidden device is never started.

| Symptom | Cause | Fix |
|---|---|---|
| Log says a device "is in a half-edited state and this plugin can no longer see it" | The **Edit Device Type** menu was used on it (see above). | **Delete** the device in Indigo, then reload the plugin. The next reconcile recreates it with the correct type. Its states and history do not survive — nothing else can, since the original is unreachable. |
| A device stopped updating and nothing appears in the log about it | Same cause, on a node that is offline or decommissioned, so nothing tries to recreate it. Since 2026.24.2 the warning above fires at every reconcile regardless. | As above. If you are on an older version, look for the device in Indigo's device list showing no recent activity. |
...
| Installing/updating either agent (controller or bridge) can't reach the new install, or seems to keep running an old Node | `nodeBinDir` is pinned to whichever node resolved at the last successful install. **Fixed in 2026.24.1** (issue #101): the pinned node is now checked before it is used, and a pin that will not run or is below Node 22.13 is reported and bypassed. On older versions the stale pin silently won, for **both** agents (they share it). | Upgrade to 2026.24.1+. If you are still on an older build, or want to pin a different node: Configure… ▸ **Show advanced server settings** ▸ clear **Node bin directory (optional):**, save, then reinstall/restart so auto-detect re-resolves. |
...
| Apple Home refuses to change an exported accessory's **room**, or every accessory jumps back into the bridge's own room after a restart | **Fixed going forward in plugin 2026.8.4 / bridge `0.6.0`** (issue #141): older bridges came online with an empty accessory list and only filled it in when the plugin connected, seconds later — Apple read that gap as "every accessory gone" and treated the ones that reappeared as brand-new, stuck in the bridge's own room, with metadata it will not let you edit. The bridge now rebuilds its accessory list from disk *before* it goes online, so a current install does not do this to itself on a restart. **Upgrading does not undo the lock on a room Apple already broke** — that needs the remedy in the next column, once, for the accessories an older version affected. | **On a current install: nothing to do** — rooms set from here on stick across reboots, upgrades and crash recovery. **To free a room Apple has already locked, the verified remedy is a full re-pair, not a room edit:** in Apple Home, **Remove Accessory** on the stuck accessory (or the whole bridge) — this clears only *one* of the two fabrics an Apple Home pairing creates, the *Apple Home* fabric; **the *Apple Keychain* (iCloud sync) fabric survives, and clearing it is the step people miss** — do that explicitly with *Unpair an Ecosystem…*, which, as the last remaining fabric, factory-resets the bridge node (expected, not a fault); then re-pair with *Pair Matter Bridge…* — the bridge's identity, and the passcode in it, persists across that reset, so the fresh window shows the same code you paired with before — and rooms are editable again once the accessory reappears. **Cost:** every name, room, scene and automation you built on the exported accessories in that ecosystem is lost, because Apple treats the re-paired bridge as brand new; there is no way to keep them. Since bridge-node **0.8.0** (issue #140) that reset no longer produces an `endpoint-number DRIFT detected` log line — the node voids its own witness numbers at the reset and silently adopts whatever matter.js hands out fresh, so **Rebuild Matter Endpoint Map… is not needed** here on a current node. On a bridge node from before 0.8.0 you will see a drift report instead — update the bridge. |
