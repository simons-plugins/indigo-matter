...
- **"Matter Bridge Health" now shows five per-fabric slots** (issue #288):
  ...
  "churning" but not WHICH — these slots name each connected fabric (its
  label, e.g. "Amazon Alexa") and each one's own health (`healthy`,
  `churning`, or `unknown`), so a trigger can target the specific ecosystem
  having trouble. Slots are positional, not tied to a fabric's real index —
  they fill in order and repack when a fabric leaves, so a slot's occupant
  can change over time; an unused slot is empty. A house with more than five
  paired ecosystems (unusual) keeps the first five by fabric index and logs
  which were left out, once per streak, rather than silently dropping them.
